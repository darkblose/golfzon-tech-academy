...
//		auth.inMemoryAuthentication().withUser("bbb@bbb.com").password(passwordEncoder().encode("hi1234")).roles("USER");
//		auth.inMemoryAuthentication().withUser("bbb@bbb.com").password(passwordEncoder().encode("hi1234")).roles("ADMIN");
	}
    ...
    protected void configure(HttpSecurity http) throws Exception {
		...
		*/
        ...
        http.csrf().disable()	// csrf 토큰을 비활성화
			...
			.antMatchers("/","/m_insert","/hello","/m_insertOK","/m_updateOK","/login/**","/logout","/js/**","/css/**","/images/**").permitAll() // 해당 경로들은 접근을 허용
			.antMatchers("/m_selectAll").hasRole("ADMIN") // ADMIN만 접근 가능
//			.antMatchers("/m_selectOne").hasRole("USER") // ADMIN,USER 접근 가능
